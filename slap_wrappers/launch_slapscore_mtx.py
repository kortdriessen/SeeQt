...
glut_events = st.checkbox("Include Glutamate Events", value=False)
...
if glut_events:
    ...
    matrix_yvals = []
    matrix_alphas = []
    glut_events_dir = f"Z:\\slap_mi\\analysis_materials\\{subject}\\{exp}\\scoring_data\\sync_block-{sb}\\glut_events\\{acq_id}"
    for f in os.listdir(glut_events_dir):
        if not f.endswith("_y.npy"):
            continue
        matrix_times.append(
            os.path.join(glut_events_dir, f.replace("_y.npy", "_t.npy"))
        )
        matrix_yvals.append(os.path.join(glut_events_dir, f))
        matrix_alphas.append(
            os.path.join(glut_events_dir, f.replace("_y.npy", "_snr.npy"))
        )
    matrix_colors = np.load(os.path.join(glut_events_dir, "dmd_colors.npy")).tolist()
